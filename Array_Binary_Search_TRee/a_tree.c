...
int main()
{
    ...
    while(scanf("%d", &tmp) == 1)
        insert(tmp);
    puts("pre_order_traverse:");
    pre_order_traverse(callback);       // 回调函数，调用时不用加入任何形参
    getchar();
    return 0;
}